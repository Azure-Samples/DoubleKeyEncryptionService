...
try {
  ...
  Write-Host "$($_.Exception.Response.Headers.ToString())"
  $headerCount = $_.Exception.Response.Headers.Count.Count

  $authFound = $False

  if ( $_.Exception.Response.Headers.WwwAuthenticate -ne "" ) {
    Write-Host "Found auth header - $($_.Exception.Response.Headers.WwwAuthenticate)"

    $responeFields = ParseAuthResponse $_.Exception.Response.Headers.WwwAuthenticate
